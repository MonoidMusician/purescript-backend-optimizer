...
import PureScript.Backend.Optimizer.Semantics (BackendExpr(..), BackendSemantics(..), Ctx(..), DataTypeMeta, Env(..), EvalRef(..), ExternImpl(..), ExternSpine, InlineAccessor(..), InlineDirective(..), InlineDirectiveMap, NeutralExpr(..), build, eval, evalExternFromImpl, evalExternRefFromImpl, freeze, lookupDirective, noDirectives, optimize, quote, unionDirectives)
...
toBackendTopLevelBindingGroup env = case _ of
  NonRec binding -> do
    ...
    res2
      { accum
          { directives = unionDirectives res2.accum.directives directives
    ...
    let
      ...
      res = mapAccumL
        ( \env' binding -> do
            ...
            { accum: res3.accum
                { directives = unionDirectives res3.accum.directives directives
...
updateEnvForBinding group env (Tuple qual expr) = do
  ...
  { accum: env
      { implementations = QIMap.insert qual impl env.implementations
      , moduleImplementations = QIMap.insert qual impl env.moduleImplementations
      }
...
toTopLevelBackendBinding env (Binding _ ident cfn) = do
  ...
  , directives: if Map.size directives > 0 then Tuple (QIMap.singleton qualifiedIdent directives) Map.empty else noDirectives
...
inferTransitiveDirective directives cfn originalExpr optimizedExpr = fromOptimized <|> fromOriginal
  ...
  fromOptimized = case optimizedExpr of
    ExprSyntax _ (App (ExprSyntax _ (Var qual)) args) ->
      case lookupDirective (EvalExtern qual) directives of
        dirs ->
          foldrWithIndex
